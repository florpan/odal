...
| `visual`       | required | `{ shape: 'cone' \| 'rock', color, models?, scale? }`. `models` lists GLBs under `/models/` at world scale (1 unit = 1 tile, base at y=0, a KayKit person is 1 tile tall); each node picks one by id, `scale` multiplies the size of all of them equally so their relative sizes survive. Without `models` the primitive is drawn                             |
...
| `visual`     | required | `{ shape: 'box' \| 'cone', color, height, glow?, model? }`. `glow` adds emissive color and a point light. `model` is a GLB under `/models/` with a 1×1 footprint and base at y=0, scaled to the footprint; `{team}` in the name becomes the owner's nearest KayKit colour (red, blue, green, yellow). Remembered enemy buildings stay primitives. |
