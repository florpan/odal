...
| `visual`       | required | `{ shape: 'cone' \| 'rock', color, models?, scale? }`. `models` lists GLBs under `/models/` (1 unit tall, base at y=0); each node picks one by id, `scale` multiplies the height. Without `models` the primitive is drawn                                                                                                                                     |
