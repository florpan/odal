...
| `visual`   | required | `{ color, height: 0, model?, shore? }`. `height` is where the tile's top sits relative to the ground plane (water below 0 gives a shore step). `model` is a GLB hex tile under `/models/`, one hex wide, top at y=0; without it a flat coloured hex is drawn. `shore` lists coast tiles for a hex of this terrain that borders impassable terrain, by number of consecutive water edges (first entry = one edge; longer runs use the last entry); they are authored with the water side towards +z and the renderer turns them. `tools/models/kaykit_prop.py` mode `hex[:deg]` makes both from the KayKit Hexagon pack. `scatter` is a list of `{ models, density, scale: 1, lift: 0, patches: 0, patchScale: 6, shore: 'any' }`: cosmetic props (grass, bushes, water plants) the client sprinkles over hexes of this terrain, `density` per hex on average at `scale`, `lift` above the tile top (negative sinks). `patches` above 0 gathers them into clumps about `patchScale` hexes across (1 = clumps with bare ground between). `shore` concerns hexes whose neighbour differs in passability (a coast tile, or the water beside it): `only` puts the props nowhere else (water plants), `none` keeps them off such hexes (grass would stand in a coast tile's water). Placement is derived from the hex index, so every client sees the same board; buildings hide what stands under them. Never part of the state |
...
| `visual`      | `{}`     | `{ width: 0.4, height: 1, helmet: false, model? }`. Without `model` the unit is a box in player colour with an optional helmet. With `model: "worker.glb"` the client loads that GLB from `/models/` (generated into `packages/client/public/models` by `tools/models`), scales it by `height` (models are authored 1 unit tall, feet at y=0, facing +z) and paints every face that carries a material named `Team` in the player colour (the rest of the model keeps its own materials). If the GLB is skinned, its animation clips are played by name from the unit's task: `idle`, `walk` (whenever it moves), `chop` / `mine` (gathering at a cone / rock node), `build`, `attack`; `hit` and `death` are reserved. `tools/models/kaykit_character.py` assembles such a file from a KayKit character and animation pack: it can drop meshes, recolour palette cells of the character's texture and mark the faces of a palette cell as `Team` (the worker's scarf and cuffs). Missing file → box. |
...
| `accepts`   | `[]`            | Resource ids a drop-off takes; empty means everything. A harvester delivers to the closest finished own drop-off that takes what it carries, chosen afresh every trip (a razed mill just means a longer walk). The default ruleset's town hall takes everything so no worker is ever stranded; one carrying something no drop-off takes stops and says so.                             |
