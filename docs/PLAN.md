...
- [ ] Replace primitives with low-poly models. Done so far (2026-09-05): units take `visual.model` (a GLB
      under `client/public/models`), the worker is a ~1100-triangle procedural labourer generated by
      `tools/models/worker.ts` (`bun run model:worker`), with `tools/models/worker_blender.py` as the
      Blender-scripted alternative and `models.html` as the viewer. The shipped worker is KayKit's Rogue
      (CC0) with eight clips renamed to task names, assembled by `tools/models/kaykit_character.py`; the
      renderer crossfades idle / walk / chop / mine / build / attack from the unit's task. The house is KayKit home_A in the owner's nearest colour variant ({team}), trees are four KayKit Forest Nature C-trees picked per node, both via tools/models/kaykit_prop.py. Next: tools in the worker's hands per clip, soldier (Knight), remaining buildings, rocks from Resource Bits, one shared texture per pack instead of one per GLB.
